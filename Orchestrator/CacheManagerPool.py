...
def CacheWorker(app_name, q: JoinableQueue, stop_event: Event, q_stream: Queue):
    ...
    while True:
        ...
        try:
            ...
            #time.sleep(0.1)
            num_req += 1
            print(f"[**CW_{pid} ({app_name})] Req: {cache_req.project_id}" + str(num_req))
            ...
            print(f"[**CW_{pid} ({app_name})] Failed to run process request {cache_req.project_id}:" + str(e))
...
class CacheManager(): 
    def __init__(self,app_name,queue_list,project_list,max_num_process,dry_run): 
        ...
        self.logging.debug(f"{self.log_header} Cache Manager Pool started [max req:{self.max_num_cache_request}]")
