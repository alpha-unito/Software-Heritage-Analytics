...
def unpackProjectFile(project_id, tar_file):
    ...
    vista_di_memoria = tar_file.getbuffer()
    dimensione_buffer = len(vista_di_memoria)

    if dimensione_buffer <= (500*1024*1024):
...
def scheduleFileOnStream(language, queue_list, file_list: Dict):
    ...
    ret += "[+] stream " + str(num_file) + " files, tot byte" + \
        str(data_size)+"\n"+"-----------------------------------\n"
    # print("-----------------------------------")
    return ret
...
def logCSV(appname, log_string_dict):
    ...
    with open(f'logs/{appname}.csv', mode='a+') as csv_file:
        fieldnames = ['call_time', 'op', 'project_id',
                      'outcome', 'execution_time', 'error']
