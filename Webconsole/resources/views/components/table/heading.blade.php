...
<th scope="col" {{ $attributes->merge(['class' => 'hidden px-3 py-3.5 text-left text-sm font-semibold text-gray-900 lg:table-cell'])->only('class') }}>
    @unless ($sortable)
        <span class="text-xs font-bold leading-4 tracking-wider text-left text-gray-900 uppercase">{{ $slot }}</span>
    @else
        <button {{ $attributes->except('class') }} class="flex items-center space-x-1 text-xs font-bold leading-4 tracking-wider text-left text-gray-900 uppercase group focus:outline-none focus:underline">
            <span>{{ $slot }}</span>
